...
import com.zyq.frechwind.base.ImageImageTwo;
        ...
        while (matcher.find()) {
            ...
            if (m.find()) {
                ...
                if (imageWrapper.getWidth() > maxWidth) {
                    //这里处理图片，把图片读出来，缩小。
                    ImageImageTwo.compress(new FileInputStream(new File(path)), new File(path),maxWidth,imageWrapper.getHeight());
                }
    ...
    public void createImgByHtml(String html,String ownerType,String ownerId,String bizType){
        ...
        while (matcher.find()) {
            ...
            if (m.find()) {
                ...
                if(list.size()==0){
                    this.uploadService.createUpload(file,file.getName(),ownerType,ownerId,bizType,null);
                }
